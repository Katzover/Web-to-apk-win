...
get_tools() {
    ...
    try "wget -q --show-progress 'https://dl.google.com/android/repository/commandlinetools-${os_type}-12266719_latest.zip' -O cmdline-tools.zip"
...
EOL
    ...
    try wget -q --show-progress https://raw.githubusercontent.com/gradle/gradle/v8.6.0/gradle/wrapper/gradle-wrapper.jar -O gradle/wrapper/gradle-wrapper.jar
    try wget -q --show-progress https://raw.githubusercontent.com/gradle/gradle/v8.6.0/gradlew -O gradlew
    try wget -q --show-progress https://raw.githubusercontent.com/gradle/gradle/v8.6.0/gradlew.bat -O gradlew.bat
    try chmod +x gradlew
...
get_java() {
    ...
    local jdk_dir="$install_dir/jdk-${jdk_version}"

    # Pick platform-specific archive
    case "$(uname -s)" in
        Linux*)
            ...
            local jdk_dir_name="jdk-${jdk_version}"
            ;;
        Darwin*)
            local jdk_hash="b85c4aaf7b141825ad3a0ea34b965e45c15d5963677e9b27235aa05f65c6df06"
            local jdk_url="https://download.java.net/java/GA/jdk17.0.2/dfd4a8d0985749f896bed50d7138ee7f/8/GPL/openjdk-17.0.2_macos-x64_bin.tar.gz"
            local jdk_dir_name="jdk-${jdk_version}.jdk/Contents/Home"
            ;;
        *)
            error "Unsupported OS for get_java. Please install Java 17 manually."
            ;;
    esac
...
check_and_find_java() {
    ...
    # Check system JVM directories (Linux: /usr/lib/jvm, macOS: /Library/Java/JavaVirtualMachines)
    local jvm_roots=()
    [ -d "/usr/lib/jvm" ] && jvm_roots+=("/usr/lib/jvm")
    [ -d "/Library/Java/JavaVirtualMachines" ] && jvm_roots+=("/Library/Java/JavaVirtualMachines")

    for jvm_root in "${jvm_roots[@]}"; do
        while IFS= read -r java_path; do
            # macOS JDKs have bin/java at Contents/Home/bin/java
            local bin_java="$java_path/bin/java"
            [ -x "$java_path/Contents/Home/bin/java" ] && bin_java="$java_path/Contents/Home/bin/java"
            if [ -x "$bin_java" ]; then
                version=$("$bin_java" -version 2>&1 | head -n 1 | cut -d'"'  -f2 | cut -d'.' -f1)
                if [ "$version" = "17" ]; then
                    local home_dir="$(dirname "$bin_java")"
                    export JAVA_HOME="$home_dir"
                    export PATH="$JAVA_HOME/bin:$PATH"
                    info "Found system Java 17: $home_dir"
                    return 0
                fi
            fi
        done < <(find "$jvm_root" -maxdepth 1 -type d)
    done
